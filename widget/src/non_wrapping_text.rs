...
use crate::core::{
    Element, Layout, Length, Pixels, Point, Rectangle, Size, Widget,
};
use crate::core::widget::text::{Appearance, StyleSheet};

use std::borrow::Cow;

pub use text::{LineHeight, Shaping};

/// Customized Text widget that doesn't wrap and always stays on one line. Overflow text is hidden.
    ...
    Renderer: text::Renderer,
{
    ...
    ///
    /// [`Font`]: crate::core::text::Renderer::Font
    ...
    Renderer: text::Renderer,
{
    layout::sized(limits, width, height, |limits| {
        ...
        let limits = limits.width(width).height(
            Length::Fixed(line_height.to_absolute(size).into())
        );
        let font = font.unwrap_or_else(|| renderer.default_font());
        let bounds = crate::core::Size::new(f32::INFINITY, limits.max().height);
    ...
    Renderer: text::Renderer,
{
    ...
    renderer.with_layer(bounds, |renderer| {
        renderer.with_translation(crate::core::Vector::new(0.0, 0.0), render)
    });
}
